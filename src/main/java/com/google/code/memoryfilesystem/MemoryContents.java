...
class MemoryContents {
	...
	public int read(ByteBuffer dst, long position) throws IOException {
		try (AutoRelease lock = this.readLock()) {
			...
			long remaining = (long) dst.remaining();
			int toRead = (int) min(min(this.size - position, remaining), (long) Integer.MAX_VALUE);
			int currentBlock = (int) (position / (long) BLOCK_SIZE);
			int startIndexInBlock = (int) (position - ((long) currentBlock * (long) BLOCK_SIZE));
			int read = 0;
			while (read < toRead) {
				int lengthInBlock = (int) min((long) BLOCK_SIZE - (long) startIndexInBlock, position + (long) toRead);
	...
	public int write(ByteBuffer src, long position) {
		try (AutoRelease lock = this.writeLock()) {
			...
			int toWrite = (int) min(remaining, (long) Integer.MAX_VALUE);
			int currentBlock = (int) (position / (long) BLOCK_SIZE);
			int startIndexInBlock = (int) (position - ((long) currentBlock * (long) BLOCK_SIZE));
			int written = 0;
			while (written < toWrite) {
				int lengthInBlock = (int) min((long) BLOCK_SIZE - (long) startIndexInBlock, position + (long) toWrite);
	...
	private void ensureCapacity(long capacity) {
		...
		if (capacity == 0L) {
			...
			blocksRequired = (int) ((long) (capacity - 1L)/ (long) BLOCK_SIZE) + 1;
		}
